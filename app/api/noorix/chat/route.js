...
const PROMPTS = {
  ...
  skinAge: BASE_RULES + '\nAnalyze skin age from selfie photos. Response: {"message":"","predictedAge":"25-30","actualAge":"30-35","ageDifference":"skin looks 5 years younger","skinMetrics":{"firmness":"","hydration":"","texture":"","pigmentation":"","elasticity":""},"rejuvenationPlan":["step"],"noorivaTip":"","actions":[{"label":"","type":"addProduct|learnMore","payload":""}]}',
  ingredientConflict: BASE_RULES + '\nCheck skincare ingredient combinations for conflicts. Response: {"message":"","productsAnalyzed":["product name"],"conflicts":[{"ingredient1":"","ingredient2":"","severity":"mild|moderate|severe","explanation":"","solution":""}],"synergies":[{"ingredient1":"","ingredient2":"","benefit":""}],"overallSafety":"safe|caution|dangerous","recommendations":[""],"noorivaTip":"","actions":[{"label":"","type":"addProduct|learnMore","payload":""}]}',
};
...
function checkRate(ip) {
  ...
  if (!rec || now - rec.t > RATE_WINDOW_MS) { rateLimitMap.set(ip, { n: 1, t: now }); return true; }
...
export async function POST(request) {
  ...
  try {
    if (!checkRate(ip)) {
      return NextResponse.json({ error: 'Too many requests. Please wait.' }, { status: 429, headers: { 'Retry-After': '60' } });
    }
    ...
    if (!type || !VALID_TYPES.includes(type)) return NextResponse.json({ error: 'Invalid feature type.' }, { status: 400 });
    if (!PROMPTS[type]) return NextResponse.json({ error: 'Feature not configured.' }, { status: 400 });
    ...
    const allMessages = messages.map(function(m) { return { role: m.role, content: sanitizeString(m.content || '') }; });

    const sanitizedData = sanitizeData(data);
    if (Object.keys(sanitizedData).length > 0) {
      const last = allMessages[allMessages.length - 1];
      const dataLines = Object.entries(sanitizedData).map(function(e) { return e[0] + ': ' + e[1]; }).join('\n');
      allMessages[allMessages.length - 1] = { role: last.role, content: last.content + '\n\nContext:\n' + dataLines };
    }
