...
function assertEnv() {
  ...
  if (missing.length > 0) {
    ...
    console.warn(
      `[auth] Missing recommended environment variables: ${detail}. Configure them in your Netlify Dashboard or .env.local.`
    );
  }
  ...
  async authorize(credentials) {
    ...
    try {
      ...
      const { data, error } = await client.auth.signInWithOtp({
        email,
        emailRedirectTo: `${authUrl.replace(/\/$/, "")}/api/auth/callback/email`,
      });
...
const googleProvider = hasGoogleAuth
  ? Google({
...
export const config = {
  ...
  secret:
    ...
    "nooriva-build-fallback-secret-key-32chars-min",
