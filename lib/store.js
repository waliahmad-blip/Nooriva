  ...
  persist(
    (set, get) => ({
      ...
      addNoorixMessage: (feature, message) =>
        set((state) => {
          const existing = state.noorixMessages[feature] || [];
          const capped = existing.length >= 50 ? existing.slice(-40) : existing;
          return {
            noorixMessages: {
              ...state.noorixMessages,
              [feature]: [...capped, message],
            },
          };
        }),
      clearNoorixMessages: (feature) =>
        ...
        setNoorixPlan: (plan) => set({ noorixPlan: plan }),
        useNoorixCredit: () =>
          set((state) => {
            const today = new Date().toDateString();
            if (state.noorixDailyDate !== today) {
              return { noorixDailyUsed: 1, noorixDailyDate: today };
            }
            return { noorixDailyUsed: state.noorixDailyUsed + 1 };
          }),
      ...
      addToCart: (id, qty = 1) => {
        try {
          if (typeof window !== "undefined" && get().soundOn) {
            import("./sound").then((m) => m.playAddBag?.());
          }
        } catch (e) {}
        return set((state) => ({
