...
const MODEL_DEFINITIONS = Object.freeze({
  main: {
    name: process.env.MODEL_MAIN || 'gemini-3.8-flash',
    role: 'main',
    capabilities: ['text', 'vision', 'multiturn'],
    streaming: false,
  },
  vision: {
    name: process.env.MODEL_VISION || 'gemini-3.8-flash',
    role: 'vision',
    capabilities: ['text', 'vision', 'multiturn'],
    streaming: false,
  },
  ...
  imageGen: {
    name: process.env.MODEL_IMAGE_GEN || 'gemini-3-pro-image',
    role: 'image-generation',
    capabilities: ['image'],
    streaming: false,
  },
  imageGenFallback: {
    name: process.env.MODEL_IMAGE_FALLBACK || 'gemini-2.5-flash-image',
    role: 'image-generation',
    capabilities: ['image'],
    streaming: false,
  },
...
/**
 * Accept service-account credentials supplied via an environment variable.
 *
 * Netlify (and any other Linux / ephemeral host) has no filesystem path to the
 * key file, so NOORIX_VERTEX_KEY must be able to carry the credentials
 * themselves. Three forms are supported:
 *   • raw JSON             { "type": "service_account", ... }
 *   • base64 of that JSON  (recommended — single line, safe in env vars)
 *   • a filesystem path    (local development only)
 */
function parseInlineCredentials(raw) {
  ...
  // Attempt base64 (strip whitespace that env-var editors sometimes inject)
  try {
    ...
    // Not base64 — fall through to path handling.
  }
...
function validateVertexConfig() {
  ...
  // 1. Inline credentials — works on any host, no filesystem required.
  const credentialsObject = parseInlineCredentials(raw);

  // 2. Otherwise fall back to a readable key file (local development).
  let credentialsFile = null;
  const candidates = [
    raw,
    process.env.GOOGLE_APPLICATION_CREDENTIALS,
    path.join(process.cwd(), 'noorix-vertex-key.json'),
    process.env.NODE_ENV !== 'production' ? 'C:\\Nooriva\\noorix-vertex-key.json' : null,
  ];
  ...
  if (!credentialsObject && !credentialsFile) {
    throw new NoorixConfigError(
      'Noorix Vertex AI credentials are not configured. Set NOORIX_VERTEX_KEY on this ' +
        'host to the base64-encoded service-account JSON (or the raw JSON), or provide a ' +
        'readable key file for local development.'
    );
  }

  return {
    projectId: credentialsObject?.project_id || projectId,
    location,
    credentialsFile,
    credentialsObject,
  };
...
function createClient() {
  const { projectId, location, credentialsFile, credentialsObject } = validateVertexConfig();
  ...
  return new GoogleGenAI({
    ...
    location,
    ...(credentialsObject
      ? { googleAuthOptions: { credentials: credentialsObject } }
      : { googleAuthOptions: { keyFilename: credentialsFile } }),
  });
}
 ...
 */
function buildUserParts(messages) {
  const last = messages[messages.length - 1];
  const parts = [];
  if (last && typeof last.image === 'string' && last.image.startsWith('data:')) {
    const match = last.image.match(/^data:(image\/[a-zA-Z0-9.+-]+);base64,(.*)$/s);
    if (match) {
      parts.push({ inlineData: { mimeType: match[1], data: match[2] } });
    }
  }
  parts.push({ text: (last?.content || '').slice(0, CONFIG.limits.maxInputLength) });
  return parts;
}
...
async function generateWithFallback({ messages, systemPrompt, featureType, useJson, startModelKey }) {
  ...
  async function attempt(modelKey) {
    // gemini-3-pro-image does not support structured JSON output;
    // ensure structured JSON calls always route to JSON-capable models (e.g. gemini-3.8-flash)
    let effectiveModelKey = modelKey;
    if (useJson && (modelKey === 'imageGen' || modelKey === 'imageGenFallback')) {
      effectiveModelKey = 'main';
    }
    const modelName = MODELS[effectiveModelKey] || MODELS.main;
    seen.add(effectiveModelKey);

    if (!breaker.allowRequest(effectiveModelKey)) {
      metrics.fallbacks++;
      throw new NoorixCircuitOpenError(`Circuit open for model "${effectiveModelKey}"`, {
        model: modelName,
        featureType,
      });
    }
    ...
    try {
      ...
      const result = await withTimeout(
        client.models.generateContent({
          model: modelName,
          contents: [...history, { role: 'user', parts: buildUserParts(messages) }],
          config,
        }),
...
export async function generateImage(prompt) {
  ...
  try {
    ...
    breaker.recordFailure(modelKey, error);
    metrics.recordCall(modelKey, Date.now() - startedAt, false);
    log.warn('Primary image generation failed, trying fallback', { modelName, error: error.message });

    // Try fallback image generation model
    try {
      const fallbackName = MODELS.imageGenFallback || 'gemini-2.5-flash-image';
      const client = getClient();
      ...
      const candidates = fallbackResult?.candidates ?? fallbackResult?.response?.candidates ?? [];
      const parts = candidates[0]?.content?.parts ?? [];
      for (const part of parts) {
        if (part?.inlineData?.data) {
          return `data:${part.inlineData.mimeType ?? 'image/png'};base64,${part.inlineData.data}`;
      ...
      log.error('Image generation fallback also failed', { error: fbErr.message });
    }
    return null;
  }
}
