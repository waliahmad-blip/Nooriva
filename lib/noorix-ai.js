...
function validateVertexConfig() {
  ...
  let credentials = process.env.NOORIX_VERTEX_KEY;
  if (!credentials || !fs.existsSync(credentials)) {
    const localKey = path.join(process.cwd(), 'noorix-vertex-key.json');
    if (fs.existsSync(localKey)) {
      credentials = localKey;
    } else if (fs.existsSync('C:\\Nooriva\\noorix-vertex-key.json')) {
      credentials = 'C:\\Nooriva\\noorix-vertex-key.json';
    }
  }
  return {
    projectId,
    location: process.env.GOOGLE_CLOUD_LOCATION || 'global',
    credentials: credentials && fs.existsSync(credentials) ? credentials : null,
  };
}
